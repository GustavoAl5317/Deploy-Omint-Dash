...
// Função para calcular as somas e criar o gráfico de pizza para os Vendas
async function createChart() {
    ...
    async function updateChart() {
        ...
        const VendasData = data.filter(obj => obj.csqname === 'Vendas');

        let totalConversa = 0;
        let totalRingando = 0;
        let totalEspera = 0;

        VendasData.forEach(obj => {
    ...
    const myChart = new Chart(ctx, {
        ...
        options: {
            plugins: {
                title: {
                    display: true,
                    text: 'Comparação entre Talktime, Holdtime e Ringtime   - Vendas'
                }
            },
            tooltips: {
                callbacks: {
