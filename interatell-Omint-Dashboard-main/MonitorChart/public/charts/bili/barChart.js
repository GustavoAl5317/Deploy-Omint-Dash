...
// Função para calcular as somas e criar o gráfico de pizza para os Bilingue
async function createChart() {
    ...
    async function updateChart() {
        ...
        const BilingueData = data.filter(obj => obj.csqname === 'Bilingue');

        let totalConversa = 0;
        let totalRingando = 0;
        let totalEspera = 0;

        BilingueData.forEach(obj => {
    ...
    const myChart = new Chart(ctx, {
        ...
        options: {
            plugins: {
                title: {
                    display: true,
                    text: 'Comparação entre Talktime, Holdtime e Ringtime  - Bilingue'
                }
            },
            tooltips: {
                callbacks: {
