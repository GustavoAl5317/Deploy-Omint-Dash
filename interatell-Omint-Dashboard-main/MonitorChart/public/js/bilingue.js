...
function atualizarValor() {
    ...
    .then(data => {
        ...
        if (data && data.length > 0) {
            ...
            const Bilingue = data.find(objeto => objeto.csqname === "Bilingue");

            // Verificar se o objeto "Empresa" foi encontrado
            if (Bilingue) {
                // Atualizar o valor na div com o valor "offered" do objeto "Bilingue"
                const offeredValue =Bilingue.offered;
                document.getElementById('classNameId').innerText = offeredValue;

                // Calcular o nível de serviço (SL) em percentagem
                const serviceLevel =Bilingue.servicelevel;
                ...
                const maxQueueTime = Bilingue.max_queuetime;
                if (maxQueueTime !== undefined) {
                    ...
                    console.error('Propriedade "max_queuetime" não encontrada ou indefinida no objeto "Bilingue".');
                }

                // TME - tempo médio de espera

                const totalTME = Bilingue.queuetime;
                const totalHandled2 = Bilingue.handled;
                ...
                if (averageServiceTimeTme !== undefined) {
                    ...
                    console.error('Propriedade "Holdtime" não encontrada ou indefinida no objeto "Bilingue".');
                }

                // total chamadas abandonadas acima de 5 segundos
                const totalAbandHigher5 = Bilingue.aband5; // Corrigido o nome da variável
                document.getElementById('classNameId-totalAband-higher5').innerText = totalAbandHigher5;

                // total chamadas atendidas
                const totalHandled = Bilingue.handled;
                document.getElementById('classNameId-handled').innerText = totalHandled;


                // maior tempo de abandono
                const maxAbandonTime = Bilingue.max_time_aband; // Corrigido o nome da variável
                ...
                // tempo de conversa
                const totalTalkTime = Bilingue.talktime;
