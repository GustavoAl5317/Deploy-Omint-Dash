...
function atualizarValor() {
    ...
    .then(data => {
        ...
        if (data && data.length > 0) {
            ...
            const Vendas = data.find(objeto => objeto.csqname === "Vendas");

            // Verificar se o objeto "Empresa" foi encontrado
            if (Vendas) {
                // Atualizar o valor na div com o valor "offered" do objeto "Vendas"
                const offeredValue =Vendas.offered;
                document.getElementById('classNameId').innerText = offeredValue;

                // Calcular o nível de serviço (SL) em percentagem
                const serviceLevel =Vendas.servicelevel;
                ...
                const maxQueueTime = Vendas.max_queuetime;
                if (maxQueueTime !== undefined) {
                    ...
                    console.error('Propriedade "max_queuetime" não encontrada ou indefinida no objeto "Vendas".');
                }

                // TME - tempo médio de espera

                const totalTME = Vendas.queuetime;
                const totalHandled2 = Vendas.handled;
                ...
                if (averageServiceTimeTme !== undefined) {
                    ...
                    console.error('Propriedade "Holdtime" não encontrada ou indefinida no objeto "Vendas".');
                }

                // total chamadas abandonadas acima de 5 segundos
                const totalAbandHigher5 = Vendas.aband5; // Corrigido o nome da variável
                document.getElementById('classNameId-totalAband-higher5').innerText = totalAbandHigher5;

                // total chamadas atendidas
                const totalHandled = Vendas.handled;
                document.getElementById('classNameId-handled').innerText = totalHandled;


                // maior tempo de abandono
                const maxAbandonTime = Vendas.max_time_aband; // Corrigido o nome da variável
                ...
                // tempo de conversa
                const totalTalkTime = Vendas.talktime;
