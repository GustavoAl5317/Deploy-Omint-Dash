...
export default async function queryAgent() {
  ...
  FROM (SELECT 
    ...
    FROM (SELECT 
      ...
      GROUP BY csqname, datetime, disposition, id, timetoaband, notaccountable, resourceid) AS subquery 
    GROUP BY csqname, total_days, resourceid) AS outerquery
    
  left join resource as r
    on outerquery.resourceid = r.resourceid`);
