...
export default async function queryTime() {
  let connection;
  ...
  try {
    ...
    from(select
      ...
      from(select
        ...
        --intervalo começo
        , to_char(ccd.startdatetime - interval(3) hour to hour, "%H") || ':' || case 
            when cast(to_char(extend (ccd.startdatetime, hour to second),'%M') as numeric) between 0 and 60 then '00'
            else '-1' end as intervalo
        --intervalo fim
